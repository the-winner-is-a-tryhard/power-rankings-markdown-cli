...
const createNewPostDirectoryName = (authorFirstName, weekText) =>
  `${authorFirstName.toLowerCase()}-week-${weekText.toLowerCase()}-power-rankings`
...
const createPathForMarkdownPost = (pathToNewBlogDirectory) => {
  return path.join(pathToNewBlogDirectory, markdownFileName)
}
